...
angular.module('tagNebulaApp')
  .controller 'NavbarCtrl', ($scope, $location, $rootScope, user, AlertService) ->
    ...
    $scope.captcha =
      ...
      answer: ''
      reload: ->
        user.get_captcha((result, status_code)=>
          @data = result
        )
    ...
    $scope.login_form =
      ...
      toggle_reg: ->
        ...
        if @login_or_reg isnt 'reg' then @login_or_reg = 'reg' else @toggle_form()
    ...
    $scope.create_user = (user_id, password)->
      $scope.err = []
      user.create_user user_id, password, $scope.captcha.data.key, $scope.captcha.answer, (err, result)->
        $scope.is_logined = user.is_logged_in()
        if err
          AlertService.push err
        else
